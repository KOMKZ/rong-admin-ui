...
| TokenPair                  | app-auth/types.ts         | Token 对，支持 `refreshAfterSeconds` 主动刷新时间点          |
| TokenStorage               | app-auth/types.ts         | 存储适配器接口                                               |
| TokenManagerInstance       | app-auth/types.ts         | Manager 实例方法，包含 `refreshNow()` 和 `getRefreshToken()` |
...
| HttpClientConfig         | app-request/types.ts       | 客户端配置；`tokenProvider.refreshToken` 可启用 401 后单飞刷新并重放请求一次 |
