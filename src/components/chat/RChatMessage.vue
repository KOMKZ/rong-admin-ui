    ...
    <div class="r-chat-message__body">
      <div class="r-chat-message__meta-row">
        ...
        <NTag
          ...
          size="small"
          :bordered="false"
          class="r-chat-message__token-badge"
        >
      ...
      <div
        v-if="message.role === 'assistant' && toolCalls.length"
        class="r-chat-message__tool-calls"
      >
        <NCollapse :default-expanded-names="[]">
          <NCollapseItem v-for="(tc, idx) in toolCalls" :key="idx" :name="`tc-${idx}`">
            <template #header>
            ...
            <div class="r-chat-message__tool-call-body">
              <div v-if="tc.arguments || tc.args" class="r-chat-message__tool-section">
                ...
                <pre class="r-chat-message__tool-pre">{{
                  formatJson((tc.arguments ?? tc.args)!)
                }}</pre>
              </div>
      ...
      <div
        v-if="message.role === 'assistant' && citations.length"
        class="r-chat-message__citations"
      >
        ...
        <div class="r-chat-message__citations-list">
          ...
          >
            ...
            <span class="r-chat-message__citation-info">
              <span class="r-chat-message__citation-title">{{
                cite.title || cite.url || 'Source'
              }}</span>
              <span v-if="cite.url" class="r-chat-message__citation-domain">{{
                extractDomain(cite.url)
              }}</span>
              <NTag
                v-if="cite.type === 'fetch'"
                size="tiny"
                :bordered="false"
                type="success"
                class="r-chat-message__citation-tag"
              >
                已认证
              </NTag>
            </span>
