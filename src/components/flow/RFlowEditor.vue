  ...
  <div class="r-flow-editor" :style="{ height }">
    <div class="r-flow-editor__toolbar">
      <slot name="toolbar">
        ...
        <button
          class="r-flow-editor__btn r-flow-editor__btn--danger"
        ...
        <button class="r-flow-editor__btn r-flow-editor__btn--primary" @click="handleSave">
          保存
        </button>
      </slot>
    </div>
  ...
  function handleSave() {
    emit('save', {
      ...
      edges: editEdges.value.map((e) => ({
        ...
        type: e.type === 'smoothstep' ? 'dispatch' : undefined,
      })),
    })
  }
</script>
...
<style scoped>
  ...
  .r-flow-editor__btn:hover {
    background: #f9fafb;
  }
  .r-flow-editor__btn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }
  .r-flow-editor__btn--primary {
    background: #6366f1;
    color: white;
    border-color: #6366f1;
  }
  .r-flow-editor__btn--primary:hover {
    background: #4f46e5;
  }
  .r-flow-editor__btn--danger {
    color: #dc2626;
    border-color: #fca5a5;
  }
  .r-flow-editor__separator {
    width: 1px;
    height: 20px;
    background: #e5e7eb;
  }
