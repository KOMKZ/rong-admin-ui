<template>
  <div class="r-workflow-designer" :style="designerStyle" data-testid="workflow-designer">
    ...
    <section class="r-workflow-canvas-wrap">
      ...
      <div ref="canvasRef" class="r-workflow-canvas" @dragover.prevent @drop="handleCanvasDrop">
        ...
        >
          ...
          <div class="r-workflow-node__actions">
            ...
            <button
              class="r-workflow-inline-btn danger"
              :disabled="readonly"
    ...
    <aside class="r-workflow-panel r-workflow-panel--inspector">
      ...
      <div v-if="inspectorTab === 'global'" class="r-workflow-inspector-section">
        <label class="r-workflow-field">
          ...
          <textarea
            ...
            @input="
              updateGlobalMetaText('description', ($event.target as HTMLTextAreaElement).value)
            "
          />
        </label>
        ...
        <div class="r-workflow-field">
          ...
          >
            <input
              ...
              placeholder="amount"
              @input="
                updateGlobalFormField(index, 'key', ($event.target as HTMLInputElement).value)
              "
            />
            <input
              :value="field.label"
              ...
              placeholder="申请金额"
              @input="
                updateGlobalFormField(index, 'label', ($event.target as HTMLInputElement).value)
              "
            />
            <select
              :value="field.type"
              :disabled="readonly"
              @change="
                updateGlobalFormField(index, 'type', ($event.target as HTMLSelectElement).value)
              "
            >
            ...
            <input
              ...
              placeholder="选项1,选项2"
              @input="
                updateGlobalFormFieldOptions(index, ($event.target as HTMLInputElement).value)
              "
            />
            <button
              class="r-workflow-inline-btn danger"
              :disabled="readonly"
              @click="removeGlobalFormField(index)"
            >
              删除
            </button>
          </div>

          <button class="r-workflow-inline-btn" :disabled="readonly" @click="appendGlobalFormField">
            新增字段
          </button>
      ...
      <div v-else-if="inspectorTab === 'node'" class="r-workflow-inspector-section">
        <template v-if="selectedNode">
          ...
          <label
            v-if="selectedNode.type === 'approval' && readConfigText('selector_mode') === 'users'"
            class="r-workflow-field"
          >
            <span>指定审批人</span>
            <select
            ...
            <p v-if="approverUsers.length === 0" class="r-workflow-field__hint">
              暂无可选用户，请先配置数据源
            </p>
          </label>

          <label
            v-if="selectedNode.type === 'approval' && readConfigText('selector_mode') === 'role'"
            class="r-workflow-field"
          >
            <span>按角色分配</span>
            <select
            ...
            <p v-if="approverRoles.length === 0" class="r-workflow-field__hint">
              暂无可选角色，请先配置数据源
            </p>
          </label>
          ...
          <label
            v-if="
              selectedNode.type === 'approval' && readConfigText('selector_mode') === 'department'
            "
            class="r-workflow-field"
          >
            <span>按部门分配</span>
            ...
            >
              <option
                v-for="department in approverDepartments"
                :key="department.id"
            ...
            <p v-if="approverDepartments.length === 0" class="r-workflow-field__hint">
              暂无可选部门，请先配置数据源
            </p>
          </label>
          ...
          <label v-if="selectedNode.type === 'cc'" class="r-workflow-field">
            ...
            <input
              ...
              :disabled="readonly"
              @input="
                updateSelectedNodeListConfig('ccUsers', ($event.target as HTMLInputElement).value)
              "
            />
          </label>
            ...
            <div class="r-workflow-rule-list">
              ...
              >
                <input
                  ...
                  @input="
                    updateConditionRule(index, 'field', ($event.target as HTMLInputElement).value)
                  "
                />
                <select
                  :value="rule.operator"
                ...
                <input
                  ...
                  placeholder="比较值"
                  @input="
                    updateConditionRule(index, 'value', ($event.target as HTMLInputElement).value)
                  "
                />
                <select
                ...
                >
                  ...
                  <option
                    v-for="candidate in conditionTargetOptions"
                    :key="candidate.id"
                    :value="candidate.id"
                  >
                    {{ candidate.label }}
          ...
          <label v-if="selectedNode.type === 'parallel'" class="r-workflow-field">
            ...
            <input
              ...
              :disabled="readonly"
              @input="
                updateSelectedNodeNumberConfig(
                  'branchCount',
                  ($event.target as HTMLInputElement).value,
                )
              "
...
<script setup lang="ts">
  ...
  const localGraph = ref<WorkflowGraph>(
    cloneGraph(props.modelValue?.nodes?.length ? props.modelValue : createDefaultGraph()),
  )
  ...
  function removeNode(nodeID: string): void {
    ...
    localGraph.value.edges = localGraph.value.edges.filter(
  ...
  function handlePanelResizeMove(event: MouseEvent): void {
    ...
    if (panelResizeState.value.side === 'left') {
      paletteWidth.value = Math.max(
        PANEL_MIN_WIDTH,
        Math.min(PANEL_MAX_WIDTH, panelResizeState.value.startWidth + deltaX),
      )
      return
    }
  ...
  function readGlobalFormFields(): WorkflowFormFieldSchema[] {
    ...
    return raw.map((item) => {
      ...
      const type = ['text', 'number', 'textarea', 'select', 'date'].includes(typeRaw)
        ? typeRaw
        : 'text'
  ...
  function updateGlobalFormField(
    index: number,
    key: keyof WorkflowFormFieldSchema,
    value: unknown,
  ): void {
    if (props.readonly) return
    const fields = readGlobalFormFields()
...
<style scoped>
  .r-workflow-designer {
    display: grid;
    grid-template-columns: var(--r-workflow-palette-width) 8px minmax(0, 1fr) 8px var(
        --r-workflow-inspector-width
      );
    gap: 12px;
  }
