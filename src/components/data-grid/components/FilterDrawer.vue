...
watch(() => props.conditions, (v) => { local.value = JSON.parse(JSON.stringify(v)) }, { immediate: true, deep: true })
watch(() => props.visible, (v) => { if (v && local.value.length === 0) addFilter() })
...
interface OpDef { label: string; value: string }

const TEXT_OPS: OpDef[] = [
  { label: '等于', value: 'equals' }, { label: '不等于', value: 'notEqual' },
  { label: '包含', value: 'contains' }, { label: '不包含', value: 'notContains' },
  { label: '开始于', value: 'startsWith' }, { label: '结束于', value: 'endsWith' },
  { label: '为空', value: 'blank' }, { label: '不为空', value: 'notBlank' },
]

const NUM_OPS: OpDef[] = [
  { label: '等于', value: 'equals' }, { label: '不等于', value: 'notEqual' },
  { label: '大于', value: 'greaterThan' }, { label: '小于', value: 'lessThan' },
  { label: '≥', value: 'greaterThanOrEqual' }, { label: '≤', value: 'lessThanOrEqual' },
  { label: '为空', value: 'blank' }, { label: '不为空', value: 'notBlank' },
]
...
function needsValue(op: string) { return op !== 'blank' && op !== 'notBlank' }

function addFilter() {
  local.value.push({ field: '', operator: '', value: null, logic: 'AND' })
}

function removeFilter(i: number) { local.value.splice(i, 1) }
function clearAll() { local.value = []; emit('update:conditions', []); emit('apply', []); emit('update:visible', false) }
function onFieldChange(i: number) { local.value[i].operator = ''; local.value[i].value = null }
        ...
        <div class="rdg-drawer rdg-drawer--right" style="width: 600px">
          ...
          <div class="rdg-drawer__body">
            <table class="rdg-filter-table">
              ...
              <tbody>
                <tr v-for="(f, i) in local" :key="i">
                  ...
                  <td>
                    <select v-model="f.field" class="rdg-select rdg-select--sm" @change="onFieldChange(i)">
                      <option value="">选择字段</option>
                      <option v-for="c in filterableColumns" :key="c.value" :value="c.value">{{ c.label }}</option>
                    </select>
                  </td>
                  <td>
                    <select v-model="f.operator" class="rdg-select rdg-select--sm">
                      <option value="">操作符</option>
                      <option v-for="o in getOps(f.field)" :key="o.value" :value="o.value">{{ o.label }}</option>
                    </select>
                  </td>
                  <td>
                    <template v-if="needsValue(f.operator)">
                      ...
                      >
                        ...
                        <option v-for="o in getSelectOptions(f.field)" :key="o" :value="o">{{ o }}</option>
                      </select>
                      <input v-else v-model="f.value" :type="getInputType(f.field)" class="rdg-input rdg-input--sm" placeholder="输入值" />
                    </template>
            ...
            <div class="rdg-filter-actions">
              ...
              <button class="rdg-btn rdg-btn--sm rdg-btn--warning" @click="clearAll">清空所有</button>
            </div>
          </div>
...
.rdg-icon-btn:hover { background: var(--ra-color-bg-muted, #f3f4f6); }
.rdg-icon-btn--danger { color: var(--ra-color-danger, #cf222e); }
</style>
