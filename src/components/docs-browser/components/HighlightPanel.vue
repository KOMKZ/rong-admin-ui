  ...
  <div class="r-docs-highlight-panel" data-testid="highlight-panel">
    <div class="r-docs-highlight-panel__header">
      ...
      <div class="r-docs-highlight-panel__actions">
        ...
        <button class="r-docs-highlight-panel__btn" title="关闭面板" @click="$emit('close')">
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
    ...
    <div class="r-docs-highlight-panel__body">
      ...
      >
        <div
          class="r-docs-highlight-item__color"
          :style="{ background: colorMap[item.color as HighlightColor] || colorMap.yellow }"
        ></div>
        <div class="r-docs-highlight-item__content">
          ...
          <template v-if="editingNoteId === item.id">
            ...
            <div class="r-docs-highlight-item__note-actions">
              <button
                class="r-docs-highlight-item__note-btn r-docs-highlight-item__note-btn--save"
                @click="$emit('save-note')"
              >
                保存
              </button>
              <button class="r-docs-highlight-item__note-btn" @click="$emit('cancel-note')">
                取消
              </button>
            </div>
          </template>
          <template v-else>
            ...
            <button class="r-docs-highlight-item__add-note" @click="$emit('edit-note', item.id)">
              {{ item.note ? '编辑备注' : '添加备注' }}
            </button>
          </template>
        </div>
